...
import { z } from "zod";
import {
  ArrowRight, CheckCircle2, Send, Quote,
} from "lucide-react";
...
import { hero, brandStory, servicesOverview, platformsSection, testimonialsSection, ctaSection, quickEnquiry } from "@/content/home";
    ...
    <>
          ...
          <div className="grid lg:grid-cols-[1.15fr_1fr] gap-10 items-center">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
              ...
              <h1 className="mt-6 text-4xl sm:text-5xl md:text-6xl font-bold tracking-tight font-heading">
                {hero.headline}<span className="bg-gradient-to-r from-primary to-white bg-clip-text text-transparent">{hero.headlineHighlight}</span>
              </h1>
      ...
      <section className="py-24 md:py-28">
        <div className="container-x grid lg:grid-cols-2 gap-14 items-center">
          ...
          <Reveal delay={0.1}>
            <span className="text-xs font-semibold tracking-widest uppercase text-primary">{brandStory.eyebrow}</span>
            <h2 className="mt-3 text-3xl md:text-5xl font-bold text-navy font-heading">{brandStory.headline}</h2>
      ...
      <section className="py-24 bg-secondary/40">
        <div className="container-x">
          <Reveal className="max-w-2xl">
            <span className="text-xs font-semibold tracking-widest uppercase text-primary">{servicesOverview.eyebrow}</span>
            <h2 className="mt-3 text-3xl md:text-5xl font-bold text-navy font-heading">{servicesOverview.headline}</h2>
        ...
        <section className="py-20">
          <div className="container-x">
            <Reveal className="text-center max-w-2xl mx-auto">
              <span className="text-xs font-semibold tracking-widest uppercase text-primary">{platformsSection.eyebrow}</span>
              <h2 className="mt-3 text-3xl md:text-4xl font-bold text-navy font-heading">{platformsSection.headline}</h2>
              ...
              {platforms.map((p, i) => (
                <Reveal key={p.name} delay={i * 0.05}>
                  <div className="flex items-center justify-center h-32">
                    <img src={platformLogoMap[p.logo]} alt={p.name} className={`max-w-full object-contain ${p.name === "Flipkart" ? "h-28" : "h-16"}`} />
                  </div>
                </Reveal>
              ))}
            </div>
        ...
        <section className="py-24 bg-secondary/40">
          <div className="container-x">
            <Reveal className="max-w-2xl">
              <span className="text-xs font-semibold tracking-widest uppercase text-primary">{testimonialsSection.eyebrow}</span>
              <h2 className="mt-3 text-3xl md:text-5xl font-bold text-navy font-heading">{testimonialsSection.headline}</h2>
            </Reveal>
          ...
          <div className="container-x">
            <Reveal>
              <div className="relative overflow-hidden rounded-3xl gradient-primary px-8 py-16 md:p-20 text-center text-primary-foreground shadow-elegant">
                ...
                <p className="mt-4 text-lg md:text-xl opacity-90 max-w-2xl mx-auto">{ctaSection.subheadline}</p>
    ...
    return (
      <div className="rounded-3xl border border-white/15 bg-white/10 backdrop-blur-xl p-6 md:p-8 shadow-elegant">
        <div className="text-white">
          <div className="text-xs font-semibold tracking-widest uppercase text-primary">{quickEnquiry.eyebrow}</div>
        ...
        {sent ? (
          <div className="mt-6 rounded-xl bg-white/10 border border-white/20 p-5 text-white">
            <div className="flex items-center gap-2 font-semibold"><CheckCircle2 className="h-5 w-5 text-primary" /> {quickEnquiry.successMessage}</div>
          ...
          <form onSubmit={onSubmit} className="mt-6 space-y-4" noValidate>
            ...
            <button type="submit" className="w-full inline-flex items-center justify-center gap-2 rounded-full gradient-primary px-6 py-3 text-sm font-bold text-primary-foreground shadow-elegant hover:scale-[1.02] transition-transform">
              {quickEnquiry.buttonLabel} <Send className="h-4 w-4" />
            </button>
