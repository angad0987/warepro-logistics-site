      ...
      <section className="relative pt-32 pb-20 md:pt-44 md:pb-28 gradient-dark text-white overflow-hidden">
        ...
        <div className="container-x relative">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="max-w-3xl">
            <span className="text-xs font-semibold tracking-widest uppercase text-primary">{blogPage.eyebrow}</span>
            <h1 className="mt-3 text-4xl md:text-6xl font-bold tracking-tight font-heading">{blogPage.headline}</h1>
          ...
          {posts.length === 0 ? (
            <Reveal>
              <div className="rounded-3xl border border-dashed border-border bg-card p-12 md:p-16 text-center shadow-card-soft">
                ...
                <h2 className="mt-6 text-2xl md:text-3xl font-bold text-navy font-heading">{blogPage.emptyHeadline}</h2>
