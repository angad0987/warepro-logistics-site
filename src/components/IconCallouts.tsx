...
export function IconCallouts({
  ...
  return (
    <section
      className={`py-24 md:py-28 relative overflow-hidden ${dark ? "gradient-dark text-white" : ""}`}
    >
      {dark && (
      ...
      <div className="container-x relative">
        <Reveal className="text-center max-w-2xl mx-auto">
          ...
          <h2 className={`mt-3 text-3xl md:text-5xl font-bold ${dark ? "" : "text-navy"}`}>
            {title}
          </h2>
          {intro && (
            <p className={`mt-4 text-lg ${dark ? "text-white/75" : "text-muted-foreground"}`}>
              {intro}
            </p>
          )}
        </Reveal>
            ...
            {whyChooseUs.map((v, i) => {
              ...
              return (
                <Reveal key={v.title} delay={i * 0.05}>
                  ...
                  >
                    <div
                      className={`grid h-20 w-20 place-items-center rounded-full ${dark ? "bg-primary/20" : "bg-primary/15"} border-2 ${dark ? "border-white/10" : "border-primary/20"}`}
                    >
                      <div className="grid h-14 w-14 place-items-center rounded-full gradient-primary text-primary-foreground shadow-glow">
                        <Icon className="h-6 w-6" />
                      </div>
                    </div>
                    <div className="mt-4 text-[11px] font-bold tracking-[0.2em] text-primary font-heading">
                      {v.microLabel}
                    </div>
                    <h3 className={`mt-1 text-lg font-bold ${dark ? "" : "text-navy"}`}>
                      {v.title}
                    </h3>
                    <ul
                      className={`mt-3 space-y-1 text-sm ${dark ? "text-white/70" : "text-muted-foreground"}`}
                    >
                      {v.bullets.map((l) => (
                        <li key={l}>{l}</li>
                      ))}
